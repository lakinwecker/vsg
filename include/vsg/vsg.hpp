...
struct node_i
{
  ...
  virtual void draw(box<ContextT> ctx) const = 0;
  [[nodiscard]] virtual auto marker() const -> std::any = 0;
  [[nodiscard]] virtual auto draw_args() const -> std::any = 0;
  [[nodiscard]] virtual auto children() const
      -> vector<box<node_i<ContextT>>> = 0;
  virtual void update_children(
      vector<box<node_i<ContextT>>> const& new_children) = 0;
...
struct node : public node_i<ContextT>
{
  ...
  node(DrawArgsT args,
       ...
       vector<box<node_i<ContextT>>> children)
      : m_draw_args {args}
  ...
  {
    return std::any(m_change_marker);
  }
  [[nodiscard]] auto draw_args() const -> std::any override
  {
    return std::any(m_draw_args);
  }

  void draw(box<ContextT> ctx) const override
  {
    using biggeo::vsg::draw;
    draw(ctx, m_gpu_data, m_draw_args);
    immer::for_each(m_children,
                    [&](const box<node_i<ContextT>>& cur_node)
                    { cur_node->draw(ctx); });
  }
  ...
  void update_children(
      vector<box<node_i<ContextT>>> const& new_children) override
  {
  ...
  {
    if (new_args.type() != typeid(m_draw_args)) {
      return;
    }
...
struct v_node_i
{
  ...
  auto operator=(v_node_i<ContextT>&&) noexcept
      -> v_node_i<ContextT>& = default;
  virtual ~v_node_i() = default;

  [[nodiscard]] virtual auto marker() const -> std::any = 0;
  [[nodiscard]] virtual auto draw_args() const -> std::any = 0;
  [[nodiscard]] virtual auto children() const
      -> vector<box<v_node_i<ContextT>>> = 0;
  [[nodiscard]] virtual auto change_marker_equals(
      node_i<ContextT> const& other) const -> bool = 0;
...
struct v_node : public v_node_i<ContextT>
{
  ...
  v_node(DrawArgsT args,
         ChangeMarkerT marker,
         vector<box<v_node_i<ContextT>>> children)
      : m_draw_args {args}
      , m_change_marker {marker}
      , m_children {std::move(children)}
  {
  ...
  [[nodiscard]] auto children() const
      -> vector<box<v_node_i<ContextT>>> override
  {
    return m_children;
  }
  [[nodiscard]] auto change_marker_equals(node_i<ContextT> const& other) const
      -> bool override
  {
    auto other_change_marker = other.marker();
    if (other_change_marker.type() != typeid(m_change_marker)) {
      return false;
    }
  ...
  [[nodiscard]] auto node() const -> box<node_i<ContextT>> override
  {
    ...
    immer::for_each(m_children,
                    [&](const box<v_node_i<ContextT>>& child) {
...
template<typename ContextT>
auto update_graph(box<node_i<ContextT>> graph,
                  box<v_node_i<ContextT>> const& v_graph)
    -> box<node_i<ContextT>>
{
  if (!graph) {
  ...
  )
  {
    ...
    while (index < v_children.size()) {
      if (index < old_children.size()) {
        ...
        new_children = new_children.push_back(
            update_graph(old_children[index], v_children[index]));
      } else {
...
inline auto update_gpu([[maybe_unused]] fragment frag)
    -> std::shared_ptr<fragment_gpu_data>
{
  return std::make_shared<fragment_gpu_data>();
}

template<typename ContextT>
void draw([[maybe_unused]] std::shared_ptr<ContextT> ctx,
          [[maybe_unused]] std::shared_ptr<fragment_gpu_data> earth,
...
template<typename ContextT, typename DrawArgsT, typename ChangeMarkerT>
auto n(DrawArgsT val,
       ChangeMarkerT marker,
       vector<box<v_node_i<ContextT>>> children) -> box<v_node_i<ContextT>>
{
  return std::make_shared<v_node<ContextT, DrawArgsT, ChangeMarkerT>>(
      val, marker, children);
}
...
inline auto operator==([[maybe_unused]] no_change_marker const& lhs,
                       [[maybe_unused]] no_change_marker const& rhs) -> bool
{
  return true;
}

template<typename ContextT, typename DrawArgsT>
auto n(DrawArgsT val, vector<box<v_node_i<ContextT>>> children)
...
auto f(ChangeMarkerT marker, vector<box<v_node_i<ContextT>>> children)
    -> box<v_node_i<ContextT>>
{
...
{
  return n<ContextT, fragment, no_change_marker>(
      fragment {}, no_change_marker {}, {});
}
